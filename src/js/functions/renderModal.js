...
const renderModalLibrary = ({ id, image, releaseDate, title, totalEpisodes, status, type, genres, description, otherName, episodes, subOrDub }) => {
  const episodeList = episodes.map(episode =>
    `<a href="${episode.url}" target="_blank" rel="noreferrer noopener" class="modal__episode">EP<span class="modal__episode-title">${episode.number}</span></a>
    `
  ).join("");
...
export {renderModal, renderModalLibrary};